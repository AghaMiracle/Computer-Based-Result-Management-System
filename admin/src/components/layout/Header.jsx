...
const Header = ({ onMenuClick, isMobile }) => {
  ...
  return (
    <header style={{
      ...
      {/* Search bar — desktop */}
      {!isMobile && (
        <div style={{ display:'flex', alignItems:'center', gap:'0.625rem', background:'#fff', border:'2px solid #000', borderRadius:'0.625rem', padding:'0.5rem 1rem', width:280, boxShadow:'3px 3px 0px #000', transition:'all 0.2s cubic-bezier(0.175,0.885,0.32,1.275)' }}
        ...
        >
          ...
          <input type="text" placeholder="Search anything..."
            style={{ border:'none', outline:'none', background:'transparent', fontFamily:"'Satoshi',sans-serif", fontSize:'0.875rem', fontWeight:500, color:'#000', width:'100%' }} />
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'0.5rem', marginLeft:'auto' }}>
        {/* Notifications */}
        <button style={{ width:38, height:38, borderRadius:'0.5rem', border:'2px solid #000', background:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#000', position:'relative', boxShadow:'2px 2px 0px #000', transition:'all 0.2s', flexShrink:0 }}
        ...
        <div ref={dropdownRef} style={{ position:'relative' }}>
          ...
          >
            <div style={{ width:30, height:30, borderRadius:'50%', background:profileOpen?'#ffe17c':'#171e19', border:'2px solid #000', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Cabinet Grotesk',sans-serif", color:profileOpen?'#000':'#fff', fontSize:'0.6875rem', fontWeight:800 }}>
              {user?.firstName?.[0]}{user?.lastName?.[0]}
            </div>
            {!isMobile && (
              <div style={{ textAlign:'left' }}>
                <div style={{ fontFamily:"'Cabinet Grotesk',sans-serif", fontSize:'0.8125rem', fontWeight:800, color:profileOpen?'#fff':'#000', lineHeight:1.2 }}>{user?.firstName} {user?.lastName}</div>
                <div style={{ fontFamily:"'Satoshi',sans-serif", fontSize:'0.625rem', fontWeight:500, color:profileOpen?'#b7c6c2':'#666', textTransform:'capitalize' }}>{user?.role}</div>
              </div>
            )}
          </button>
