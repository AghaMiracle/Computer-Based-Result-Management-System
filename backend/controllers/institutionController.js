...
export const createDepartment = async (req, res) => {
  try {
    const department = await Department.create({ ...req.body, institutionId: req.user.institutionId });
...
export const createCourse = async (req, res) => {
  try {
    const course = await Course.create({ ...req.body, institutionId: req.user.institutionId });
...
export const getPendingResults = async (req, res) => {
  try {
    const results = await Result.find({ institutionId: req.user.institutionId, status: 'submitted' })
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch pending results', error: error.message });
  }
};
...
export const approveResult = async (req, res) => {
  try {
    ...
    await Notification.create({
      recipientId: result.studentId, institutionId: req.user.institutionId,
      title: 'Result Published', message: 'Your result has been approved and published.',
      type: 'result'
    });
...
export const bulkApproveResults = async (req, res) => {
  try {
    ...
    const result = await Result.updateMany(
      { institutionId: req.user.institutionId, courseId, semesterId, status: 'submitted' },
      { status: 'approved', approvedAt: new Date(), approvedBy: req.user._id }
    );
    res.status(200).json({ success: true, message: `${result.modifiedCount} results approved` });
  } catch (error) {
